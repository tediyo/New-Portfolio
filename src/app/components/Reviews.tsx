        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {reviews.map((review, index) => (
            <CardContainer key={index} className="w-full">
              <CardBody className="bg-white relative border border-black/[0.1] w-full h-full rounded-xl p-6">
                <CardItem
                  translateZ="50"
                  className="text-xl font-bold text-neutral-600 mb-2"
                >
                  {review.name}
                </CardItem>
                <CardItem
                ...
                <CardItem
                  translateZ="40"
                  className="absolute -top-4 -right-4 w-16 h-16 rounded-full overflow-hidden border-2 border-white shadow-lg"
                >
                  <img
                    src={review.image}
                    alt={review.name}
